...
export function Card({ data, isActive, onSwipe }: CardProps) {
  ...
  const bind = useDrag(({ active, movement: [mx, my], direction: [xDir], velocity, last }) => {
    const trigger = velocity > 0.2;
    const dir = xDir < 0 ? -1 : 1;

    if (!active && last) {
      if (Math.abs(mx) > 100 || trigger) { // Threshold for swipe action
  ...
  return (
    ...
    >
      <img src={data.image} alt={data.name} className="w-full h-[416px] object-cover" />
      <animated.div 
